...
function AppShareTarget() {
  ...
  const queueComposerTextAndLaunchApp = React.useCallback(
    (text: string) => {
  ...
  React.useEffect(() => {
    ...
    else setErrorMessage('No text or url. Received: ' + JSON.stringify(query));
  }, [query.url, query.text, query]);

  // Text -> Composer
  React.useEffect(() => {
    if (intentText) queueComposerTextAndLaunchApp(intentText);
    ...
    >
      ...
      {errorMessage && (
        <>
          ...
          <Button variant="solid" color="danger" onClick={() => routerPush('/')} endDecorator={<ArrowBackIcon />} sx={{ mt: 2 }}>
            Cancel
          </Button>
        </>
      )}
      ...
      {/* URL under analysis */}
      <Typography level="body-xs">{intentURL}</Typography>
    </Box>
  );
}
