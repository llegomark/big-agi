...
const useSharingStore = createWithEqualityFn<StoreSharing>()(
  persist(
    (set) => ({
      ...
      chatLinkItems: [],
      addChatLinkItem: (chatTitle: string | undefined, objectId: string, createdAt: Date, expiresAt: Date | null, deletionKey: string) =>
        set((state) => ({
          chatLinkItems: [
            ...state.chatLinkItems,
            { chatTitle, objectId, createdAt: createdAt.toISOString(), expiresAt: expiresAt?.toISOString() ?? null, deletionKey },
          ],
        })),
      removeChatLinkItem: (objectId: string) =>
        set((state) => ({
