...
export function ExportedPublish(props: { onClose: () => void; response: PublishedSchema; open: boolean }) {
  if (!props.response?.url) return null;
  ...
  return (
    <Modal open={props.open} onClose={props.onClose}>
      <ModalDialog variant="outlined" color="neutral" sx={{ maxWidth: '100vw' }}>
        <Typography level="title-lg">Your conversation is live!</Typography>

        <Divider sx={{ my: 2 }} />

        <Typography>This is your link (opens in a new window)</Typography>
        <Typography sx={{ mt: 1 }}>
        ...
        {expires?.length >= 10 && <Typography sx={{ mt: 1 }}>The conversation will be deleted on {new Date(expires).toLocaleString()}.</Typography>}
