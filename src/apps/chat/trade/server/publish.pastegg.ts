 ...
 */
export async function postToPasteGGOrThrow(
  title: string,
  fileName: string,
  fileContent: string,
  origin: string,
  expireDays: number = 30,
): Promise<PasteGGWire.PasteResponse> {
  ...
  return await fetchJsonOrTRPCError<PasteGGWire.PasteResponse, PasteGGWire.PasteRequest>(
