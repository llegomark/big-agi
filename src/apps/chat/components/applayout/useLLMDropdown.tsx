...
function AppBarLLMDropdown(props: { llms: DLLM[]; llmId: DLLMId | null; setLlmId: (llmId: DLLMId | null) => void; placeholder?: string }) {
  ...
  for (const llm of props.llms) {
    if (!llm.hidden || llm.id === props.llmId) {
      if (!prevSourceId || llm.sId !== prevSourceId) {
        if (prevSourceId) llmItems[`sep-${llm.id}`] = { type: 'separator', title: llm.sId };
...
export function useChatLLMDropdown() {
  // external state
  const { llms, chatLLMId, setChatLLMId } = useModelsStore(
    (state) => ({
      llms: state.llms,
  ...
  const chatLLMDropdown = React.useMemo(() => <AppBarLLMDropdown llms={llms} llmId={chatLLMId} setLlmId={setChatLLMId} />, [llms, chatLLMId, setChatLLMId]);
