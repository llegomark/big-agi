...
function PrimitiveRender({ name, value }: { name: string; value: string | number | boolean | symbol | null | undefined }) {
  if (value === null || value === undefined)
    return (
      <StateLine>
        <b>{name}</b>: <i>{value === null ? 'null' : 'undefined'}</i>
      </StateLine>
    );
  else if (typeof value === 'string')
    return (
      <StateLine>
        <b>{name}</b>: &lsquo;{value}&rsquo;
      </StateLine>
    );
...
function ListRenderer({ name, list }: { name: string; list: any[] }) {
  return (
    <StateLine>
      <b>{name}</b>[{list.length ? list.length : ''}]: {list.length ? '(not displayed)' : 'empty'}
    </StateLine>
  );
}
...
function ObjectRenderer({ name }: { name: string }) {
  return (
    <StateLine>
      <b>{name}</b>: <i>object not displayed</i>
    </StateLine>
  );
}

function StateRenderer(props: { state: object }) {
  if (typeof props.state !== 'object') return <pre>Developer Warning: state is not an object: {JSON.stringify(props.state, null, 2)}</pre>;
      ...
      <Sheet>
        ...
        {entries.map(([key, value]) =>
          isPrimitive(value) ? (
            ...
            <Typography key={'state-' + key} level="body-sm">
              {key}: {value}
            </Typography>
          ),
        )}
...
export function Ephemerals(props: { conversationId: string | null; sx?: SxProps }) {
  ...
  return (
    ...
    >
      {ephemerals.map(
        (ephemeral, i) => props.conversationId && <EphemeralItem key={`ephemeral-${i}`} conversationId={props.conversationId} ephemeral={ephemeral} />,
      )}
    </Sheet>
  );
}
