...
export const MessagesSelectionHeader = (props: {
  ...
  >
    ...
    <Button
      ...
      sx={{ ml: 'auto', mr: 'auto', minWidth: 150 }}
 ...
 */
export function CleanerMessage(props: {
  message: DMessage;
  isBottom: boolean;
  selected: boolean;
  remainingTokens?: number;
  onToggleSelected?: (messageId: string, selected: boolean) => void;
}) {
  // derived state
  const {
    id: messageId,
  ...
  const handleCheckedChange = (event: React.ChangeEvent<HTMLInputElement>) => props.onToggleSelected && props.onToggleSelected(messageId, event.target.checked);

  return (
    <ListItem
      sx={{
        display: 'flex',
        flexDirection: !fromAssistant ? 'row' : 'row',
        alignItems: 'center',
        gap: { xs: 1, sm: 2 },
        px: { xs: 1, md: 2 },
        py: 2,
    ...
    >
      ...
      <Box sx={{ display: { xs: 'none', sm: 'flex' }, minWidth: { xs: 40, sm: 48 }, justifyContent: 'center' }}>{avatarEl}</Box>
