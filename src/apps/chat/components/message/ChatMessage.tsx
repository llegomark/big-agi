...
export function makeAvatar(
  ...
  messageRole: DMessage['role'] | string,
  messageOriginLLM: string | undefined,
  messagePurposeId: SystemPurposeId | undefined,
  ...
  switch (messageRole) {
    ...
    case 'assistant':
      ...
      if (messageOriginLLM === 'prodia') return <PaletteOutlinedIcon sx={iconSx} />;
        ...
        return (
          <Box
            sx={{
              ...
              textAlign: 'center',
              width: '100%',
              minWidth: `${iconSx.width}px`,
              lineHeight: `${iconSx.height}px`,
            }}
          >
            {symbol}
          </Box>
...
function explainErrorInMessage(text: string, isAssistant: boolean, modelId?: string) {
  ...
  if (!isAssistantError) return { errorMessage, isAssistantError };
  ...
  if (text.includes('"cf_service_unavailable"')) {
    errorMessage = (
      <>
        The OpenAI servers appear to be having trouble at the moment. Kindly follow the{' '}
        <Link noLinkStyle href="https://status.openai.com/" target="_blank">
          OpenAI Status
        </Link>{' '}
        page for up to date information, and at your option try again.
      </>
    );
  }
  // ...
    ...
    errorMessage = (
      <>
        The model appears to be occupied at the moment. Kindly select <b>GPT-3.5 Turbo</b>, or give it another go by selecting <b>Run again</b> from the message
        menu.
      </>
    );
    ...
    errorMessage = (
      <>
        This thread <b>surpasses the maximum size</b> allowed for {modelId || 'this model'}. {usedText}. Please consider removing some earlier messages from the
        conversation, start a new conversation, choose a model with larger context, or submit a shorter new message.
      </>
    );
  }
  // [OpenAI] {"error":{"message":"Incorrect API key provided: ...","type":"invalid_request_error","param":null,"code":"invalid_api_key"}}
    ...
    errorMessage = (
      <>
        The API key appears to be incorrect or to have expired. Please{' '}
        <Link noLinkStyle href="https://platform.openai.com/account/api-keys" target="_blank">
          check your API key
        </Link>{' '}
        and update it in <b>Models</b>.
      </>
    );
...
function useSanityTextDiffs(text: string, diffText: string | undefined, enabled: boolean) {
  ...
  React.useEffect(() => {
    if (!diffText || !enabled) return setDiffs(null);
    setDiffs(
      cleanupEfficiency(
        makeDiff(diffText, text, {
          timeout: 1,
          checkLines: true,
        }),
        4,
...
export function ChatMessage(props: {
  ...
  // external state
  const { cleanerLooks, renderMarkdown, doubleClickToEdit } = useUIPreferencesStore(
    (state) => ({
  ...
  const handleTextEdited = (editedText: string) => {
    setIsEditing(false);
    if (props.onMessageEdit && editedText?.trim() && editedText !== messageText) props.onMessageEdit(editedText);
  };
  ...
  const openSelectionMenu = React.useCallback(
  ...
  const handleMouseUp = React.useCallback(
    (event: MouseEvent) => {
      ...
      if (selection && selection.rangeCount > 0) {
        ...
        if (selectedText.length > 0) openSelectionMenu(event, selectedText);
      }
    },
    [openSelectionMenu],
  );
  ...
  return (
    ...
    >
      ...
      {!!opsMenuAnchor && (
        <CloseableMenu placement="bottom-end" sx={{ minWidth: 280 }} open anchorEl={opsMenuAnchor} onClose={closeOperationsMenu}>
      ...
      {!!selMenuAnchor && (
        <CloseableMenu placement="bottom-start" sx={{ minWidth: 220 }} open anchorEl={selMenuAnchor} onClose={closeSelectionMenu}>
          <MenuItem onClick={handleOpsCopy} sx={{ flex: 1 }}>
            <ListItemDecorator>
              <ContentCopyIcon />
            </ListItemDecorator>
