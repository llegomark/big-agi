...
const ReactMarkdown = React.lazy(async () => {
  const [markdownModule, remarkGfmModule] = await Promise.all([import('react-markdown'), import('remark-gfm')]);

  // Pass the dynamically imported remarkGfm as children
  const ReactMarkdownWithRemarkGfm = (props: any) => <markdownModule.default remarkPlugins={[remarkGfmModule.default]} {...props} />;
