...
export function ChatMessageList(props: {
  ...
  const handleTextDiagram = async (messageId: string, text: string) => {
    ...
    } else return Promise.reject('No conversation');
  };
  ...
  const handleSelectAll = (selected: boolean) => {
    ...
    if (selected) for (const message of messages) newSelected.add(message.id);
  ...
  const handleSelectionDelete = () => {
    if (props.conversationId) for (const selectedMessage of selectedMessages) deleteMessage(props.conversationId, selectedMessage);
  ...
  const { diffMessage, diffText } = React.useMemo(() => {
    ...
    if (msgB?.text && msgA?.text && !msgB?.typing) {
      const textA = msgA.text,
        textB = msgB.text;
      const lenA = textA.length,
        lenB = textB.length;
      if (lenA > 80 && lenB > 80 && lenA > lenB / 3 && lenB > lenA / 3) return { diffMessage: msgB, diffText: textA };
    }
      ...
      {filteredMessages.map((message, idx) =>
        props.isMessageSelectionMode ? (
          <CleanerMessage
            ...
            message={message}
            isBottom={idx === 0}
            remainingTokens={(chatLLM ? chatLLM.contextTokens : 0) - historyTokenCount}
            selected={selectedMessages.has(message.id)}
            onToggleSelected={handleSelectMessage}
          />
        ) : (
          <ChatMessage
            key={'msg-' + message.id}
