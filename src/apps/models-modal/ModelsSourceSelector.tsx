...
function vendorIcon(vendor: IModelVendor | null, greenMark: boolean) {
  ...
  if (vendor?.Icon) {
    if (typeof vendor.Icon === 'string') icon = <Avatar sx={{ width: 24, height: 24 }}>{vendor.Icon}</Avatar>;
    else icon = <vendor.Icon />;
  }
...
export function ModelsSourceSelector(props: { selectedSourceId: DModelSourceId | null; setSelectedSourceId: (sourceId: DModelSourceId | null) => void }) {
  // state
  ...
  const handleAddSourceFromVendor = React.useCallback(
          ...
          return {
            ...
            component: (
              <MenuItem key={vendor.id} disabled={!enabled} onClick={() => handleAddSourceFromVendor(vendor.id)}>
                <ListItemDecorator>{vendorIcon(vendor, !!vendor.hasBackendCap && vendor.hasBackendCap())}</ListItemDecorator>
                {vendor.name}
                {/*{sourceCount > 0 && ` (added)`}*/}
              </MenuItem>
            ),
