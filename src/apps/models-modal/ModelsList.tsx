    ...
    <ListItem>
      <ListItemButton onClick={() => openLayoutLLMOptions(llm.id)} sx={{ alignItems: 'center', gap: 1 }}>
        ...
        <Tooltip title={tooltip}>
          <Typography sx={llm.hidden ? { color: 'neutral.plainDisabledColor' } : undefined}>{label}</Typography>
        </Tooltip>
        ...
        {props.chipChat && (
          <Chip size="sm" variant="plain" sx={{ boxShadow: 'sm' }}>
            chat
          </Chip>
        )}
...
export function ModelsList(props: { filterSourceId: DModelSourceId | null }) {
  // external state
  const { chatLLMId, fastLLMId, funcLLMId, llms } = useModelsStore(
    (state) => ({
