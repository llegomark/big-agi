...
function VendorLLMOptions(props: { llmId: DLLMId }) {
  ...
  if (!llm) return 'Options issue: LLM not found for id ' + props.llmId;

  // get vendor
  const vendor = findVendorById(llm._source.vId);
  if (!vendor) return 'Options issue: Vendor not found for LLM ' + props.llmId + ', source ' + llm._source.id;
...
export function LLMOptionsModal(props: { id: DLLMId }) {
  ...
  // external state
  const { llm, removeLLM, updateLLM, isChatLLM, setChatLLMId, isFastLLM, setFastLLMId, isFuncLLM, setFuncLLMId } = useModelsStore(
    (state) => ({
  ...
  if (!llm) return <>Options issue: LLM not found for id {props.id}</>;
  ...
  return (
    <GoodModal
      title={
        <>
          <b>{llm.label}</b> options
        </>
      }
      open={!!props.id}
      onClose={closeLayoutLLMOptions}
      startButton={
    ...
    >
      ...
      <FormControl orientation="horizontal" sx={{ flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <ButtonGroup orientation="horizontal" size="sm" variant="outlined">
          <Button variant={isChatLLM ? 'solid' : undefined} onClick={() => setChatLLMId(isChatLLM ? null : props.id)}>
            Chat
          </Button>
          <Button variant={isFastLLM ? 'solid' : undefined} onClick={() => setFastLLMId(isFastLLM ? null : props.id)}>
            Fast
          </Button>
          <Button variant={isFuncLLM ? 'solid' : undefined} onClick={() => setFuncLLMId(isFuncLLM ? null : props.id)}>
            Func
          </Button>
        </ButtonGroup>
      </FormControl>
      ...
      <FormControl orientation="horizontal" sx={{ flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <Switch
          checked={!llm.hidden}
          onChange={handleLlmVisibilityToggle}
