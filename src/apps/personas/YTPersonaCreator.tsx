...
function useTranscriptFromVideo(videoID: string | null) {
  const { data, isFetching, isError, error } = apiQuery.ytpersona.getTranscript.useQuery(
    { videoId: videoID || '' },
    {
      enabled: !!videoID,
...
export function YTPersonaCreator() {
  ...
  const { transcript, thumbnailUrl, title, isFetching, isError, error: transcriptError } = useTranscriptFromVideo(videoID);
  React.useEffect(() => setPersonaTranscript(transcript), [transcript]);

  // use the transformation sequence to create a persona
  const { isFinished, isTransforming, chainProgress, chainIntermediates, chainStepName, chainOutput, chainError, abortChain } = useLLMChain(
    YouTubePersonaSteps,
    diagramLlm?.id,
    personaTranscript ?? undefined,
  );
    ...
    <>
      ...
      <form onSubmit={handleFetchTranscript}>
        <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
          ...
          />
          <Button type="submit" variant="solid" disabled={isFetching || isTransforming} loading={isFetching} sx={{ minWidth: 120 }}>
            Create
          </Button>
        </Box>
      </form>
        ...
        <Card sx={{ mt: 2, boxShadow: 'md' }}>
          <CardContent>
            ...
            <Box>
              {!!thumbnailUrl && (
                <picture>
                  <img src={thumbnailUrl} alt="YouTube Video Image" height={80} style={{ float: 'left', marginRight: 8 }} />
                </picture>
              )}
              <Typography level="body-sm">{personaTranscript.slice(0, 280)}...</Typography>
            </Box>
          </CardContent>
        </Card>
      )}
      ...
      {chainOutput && (
        <Box sx={{ mt: 2 }}>
          <Typography level="title-lg">YouTuber Persona System Prompt</Typography>
            ...
            >
              ...
              <Tooltip title="Copy system prompt" variant="solid">
                <IconButton
                  variant="outlined"
                  color="neutral"
                  onClick={() => copyToClipboard(chainOutput)}
                  sx={{
                    position: 'absolute',
                    right: 0,
                    zIndex: 10,
              ...
              <Typography level="body-sm">{chainOutput}</Typography>
            </CardContent>
          </Card>
        </Box>
      )}
      ...
      {chainIntermediates && chainIntermediates.length > 0 && (
        <Box sx={{ mt: 2 }}>
          <Typography level="title-lg">{isTransforming ? 'Working...' : 'Intermediate Work'}</Typography>
              ...
              <Grid xs={12} sm={6} md={4} key={i}>
                <Card>
                  <CardContent>
                    ...
                    <Typography level="body-sm">{intermediate?.slice(0, 140)}...</Typography>
                  </CardContent>
                </Card>
              </Grid>
            ))}
      ...
      {isTransforming && (
        <GoodModal open>
          ...
          <Typography level="title-sm">
            This may take 1-2 minutes. Do not close this window or the progress will be lost. If you experience any errors (e.g. LLM timeouts, or context
            overflows for larger videos) please try again with faster/smaller models.
          </Typography>
