...
export function AppChatLinkMenuItems() {
  // external state
  const { showSystemMessages, setShowSystemMessages, renderMarkdown, setRenderMarkdown, zenMode, setZenMode } = useUIPreferencesStore(
    (state) => ({
    ...
    <>
      <MenuItem onClick={() => setShowSystemMessages(!showSystemMessages)} sx={{ justifyContent: 'space-between' }}>
        ...
        <Switch
          ...
          onChange={handleRenderSystemMessageChange}
