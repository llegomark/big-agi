...
export function AppChatLinkDrawerItems() {
  ...
  return (
    <>
      <MenuItem onClick={closeLayoutDrawer} component={Link} href={getHomeLink()} noLinkStyle>
        <ListItemDecorator>
          <ArrowBackIcon />
        </ListItemDecorator>
        {Brand.Title.Base}
      ...
      {notEmpty && (
        <ListItem>
          <Typography level="body-sm">Links shared by you</Typography>
        </ListItem>
      )}
        ...
        <Box sx={{ overflowY: 'auto' }}>
          {chatLinkItems.map((item) => (
            <MenuItem
              ...
              component={Link}
              href={getChatLinkRelativePath(item.objectId)}
              noLinkStyle
              sx={{
                display: 'flex',
            ...
            >
              <Typography level="title-sm">{item.chatTitle || 'Untitled Chat'}</Typography>
