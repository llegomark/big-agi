  ...
  return (
    <Card sx={{ width: '100%' }}>
      <CardContent sx={{ flexDirection: 'row' }}>
        ...
        <ListItemDecorator>{props.hasIssue ? <WarningIcon color="warning" /> : <CheckIcon color="success" />}</ListItemDecorator>
      </CardContent>
    </Card>
  );
}
...
export function CallWizard(props: { strict?: boolean; conversationId: string; children: React.ReactNode }) {
  ...
  if (!novel && fatalGood) return props.children;
  ...
  const handleFinishButton = () => {
    if (!allGood) return navigateBack();
    touch();
  };
  ...
  return (
    <>
      ...
      <StatusCard
        ...
        button={
          overriddenRecognition ? undefined : (
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', width: '100%', gap: 2, px: 0.5 }}>
        <Typography level="body-lg">{allGood ? 'Ready, Set, Call' : 'Please resolve the issues above before proceeding with the call'}</Typography>

        <IconButton
          size="lg"
          variant={allGood ? 'soft' : 'solid'}
