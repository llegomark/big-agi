...
export const googleSearchRouter = createTRPCRouter({
   ...
   */
  search: publicProcedure.input(inputSchema).query(async ({ input }): Promise<{ pages: Search.API.BriefResult[] }> => {
    ...
    if (!customSearchParams.key || !customSearchParams.cx) throw new Error('Missing API Key or Custom Search Engine ID');
