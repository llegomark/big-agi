...
export function ProdiaSettings() {
  ...
  const {
    ...
    (state) => ({
      ...
      modelGen: state.prodiaModelGen,
      setModelGen: state.setProdiaModelGen,
  ...
  const {
    ...
    isError,
    error,
  } = apiQuery.prodia.listModels.useQuery(
    { prodiaKey: apiKey },
    {
      enabled: isValidKey,
  ...
  return (
    <>
      ...
      <FormControl orientation="horizontal" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Select
          variant="outlined"
          placeholder={isValidKey ? 'Select a model' : 'Enter API Key'}
          value={modelId}
      ...
      <FormControl orientation="horizontal" sx={{ justifyContent: 'space-between' }}>
        ...
        <Slider
          ...
          value={steps}
          onChange={(_event, value) => setSteps(value as number)}
          min={10}
          max={50}
          step={1}
          defaultValue={25}
      ...
      {advanced.on && !selectedIsXL && (
        <FormControl orientation="horizontal" sx={{ justifyContent: 'space-between' }}>
          ...
          <RadioGroup
            ...
            onChange={(e) => setProdiaAspectRatio(e.target.value as 'square' | 'portrait' | 'landscape')}
          >
      ...
      {advanced.on && (
        <FormControl orientation="horizontal" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          <FormLabelStart title="Noise Seed" description={seed ? 'Custom' : 'Random'} tooltip="Set value for reproducible images. Different by default." />
          <Input
