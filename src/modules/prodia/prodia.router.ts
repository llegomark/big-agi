...
export const prodiaRouter = createTRPCRouter({
  ...
  imagine: publicProcedure.input(imagineInputSchema).query(async ({ input }) => {
    // timeout, in seconds
    const timeout = 20;
    ...
    while (j.status !== 'succeeded' && j.status !== 'failed' && Date.now() - tStart < timeout * 1000) {
      ...
      if (sleepDelay >= 300) sleepDelay /= 2;
    }

    // check for success
    const elapsed = Math.round((Date.now() - tStart) / 100) / 10;
    if (j.status !== 'succeeded' || !j.imageUrl) throw new Error(`Prodia image generation failed within ${elapsed}s`);
  ...
  listModels: publicProcedure.input(modelsInputSchema).query(async ({ input }) => {
    ...
    if (hardcodedRemoved.length) console.warn(`Prodia models now removed from the API: ${hardcodedRemoved.map((m) => m.id).join(', ')}`);
    ...
    if (missingHardcodedIDs.length) {
      ...
      hardcodedExisting.push(
        ...missingHardcodedIDs.map((id) => {
...
async function createGenerationJob<TJobRequest extends JobRequestBase>(
...
function prodiaAccess(_prodiaKey: string | undefined, apiPath: string): { headers: HeadersInit; url: string } {
  ...
  if (!prodiaKey) throw new Error('Missing Prodia API Key. Add it on the UI (Setup) or server side (your deployment).');
