        ...
        <React.Fragment key={style.type}>
          <ListItem>
            <ListItemButton onClick={() => !readonly && props.onSelectedStyle(style.type)}>
              <ListItemDecorator sx={{ fontSize: '16pt' }}>{style.emoji}</ListItemDecorator>
              <ListItemContent>
                <Typography>{style.name}</Typography>
                <Typography level="body-sm">{style.description}</Typography>
              </ListItemContent>
            </ListItemButton>
          </ListItem>
...
function FlatteningProgress(props: { llmLabel: string }) {
  return (
    <Box sx={{ mx: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
      <CircularProgress />
      <Typography>Flattening...</Typography>
      <Typography level="body-sm">This may take up to a minute.</Typography>
      <Typography level="body-xs">Using: {props.llmLabel}</Typography>
    </Box>
  );
}
...
export function FlattenerModal(props: { conversationId: string | null; onClose: () => void }) {
  ...
  return (
    <GoodModal open={!!props.conversationId} dividers title={!selectedStyle ? 'Compression' : 'Flattening...'} onClose={props.onClose}>
      {/* Style selector */}
      <FormControl>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flexGrow: 1 }}>
          ...
          {isDone && !isError && (
            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1, alignItems: 'center', justifyContent: 'space-between' }}>
              <IconButton variant={isError ? 'solid' : 'plain'} color={isError ? 'danger' : 'primary'} onClick={handleErrorRetry}>
                <ReplayIcon />
              </IconButton>
