...
export async function openaiStreamingRelayHandler(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    if (SERVER_DEBUG_WIRE) console.log('-> streaming:', debugGenerateCurlCommand('POST', headersUrl.url, headersUrl.headers, body));
    ...
    return new NextResponse(
      `[Issue] ${access.dialect}: ${fetchOrVendorError}` + (process.env.NODE_ENV === 'development' ? ` · [URL: ${headersUrl?.url}]` : ''),
      { status: 500 },
    );
  }
  ...
  const chatResponseStream = (upstreamResponse.body || createEmptyReadableStream()).pipeThrough(
    createEventStreamTransformer(vendorStreamParser, eventStreamFormat, access.dialect),
  );
...
function createOpenAIStreamParser(): AIStreamParser {
  ...
  return (data: string) => {
    ...
    if (json.choices.length !== 1) {
      ...
      if (json.id === '' && json.object === '' && json.model === '') return { text: '', close: false };
...
function createEventStreamTransformer(
  ...
  return new TransformStream({
    start: async (controller): Promise<void> => {
      ...
      if (inputFormat === 'sse') eventSourceParser = createEventsourceParser(onNewEvent);
      else if (inputFormat === 'json-nl') eventSourceParser = createJsonNewlineParser(onNewEvent);
    },
