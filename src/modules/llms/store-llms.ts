...
export const useModelsStore = createWithEqualityFn<ModelsData & ModelsActions>()(
  persist(
    (set) => ({
      llms: [],
      sources: [],
      ...
      setChatLLMId: (id: DLLMId | null) => set((state) => updateSelectedIds(state.llms, id, state.fastLLMId, state.funcLLMId)),

      setFastLLMId: (id: DLLMId | null) => set((state) => updateSelectedIds(state.llms, state.chatLLMId, id, state.funcLLMId)),

      setFuncLLMId: (id: DLLMId | null) => set((state) => updateSelectedIds(state.llms, state.chatLLMId, state.fastLLMId, id)),
      ...
      setLLMs: (llms: DLLM[], sourceId: DModelSourceId, preserveExpired?: boolean) =>
        set((state) => {
          const otherLlms = preserveExpired === true ? state.llms : state.llms.filter((llm) => llm.sId !== sourceId);
      ...
      updateLLM: (id: DLLMId, partial: Partial<DLLM>) =>
        set((state) => ({
          llms: state.llms.map((llm: DLLM): DLLM => (llm.id === id ? { ...llm, ...partial } : llm)),
        })),

      updateLLMOptions: <TLLMOptions>(id: DLLMId, partialOptions: Partial<TLLMOptions>) =>
        set((state) => ({
          llms: state.llms.map((llm: DLLM): DLLM => (llm.id === id ? { ...llm, options: { ...llm.options, ...partialOptions } } : llm)),
        })),
      ...
      updateSourceSetup: <TSourceSetup>(id: DModelSourceId, partialSetup: Partial<TSourceSetup>) =>
        set((state) => ({
          sources: state.sources.map(
            (source: DModelSource): DModelSource =>
              source.id === id
                ? {
                    ...source,
    ...
    }),
    {
      ...
      migrate: (state: any, fromVersion: number): ModelsData & ModelsActions => {
        ...
        if (state && fromVersion === 0)
          for (const llm of state.llms) if (!llm.maxOutputTokens) llm.maxOutputTokens = Math.round((llm.contextTokens || 4096) / 2);
...
function findLlmIdBySuffix(llms: DLLM[], suffixes: string[], fallbackToFirst: boolean): DLLMId | null {
  ...
  for (const suffix of suffixes) for (const llm of llms) if (llm.id.endsWith(suffix)) return llm.id;
