...
export function OobaboogaSourceSetup(props: { sourceId: DModelSourceId }) {
  // external state
  const { source, sourceHasLLMs, access, updateSetup } = useSourceSetup(props.sourceId, ModelVendorOoobabooga.getAccess);
  ...
  return (
    <>
      <Typography level="body-sm">
        ...
        <Link href="https://github.com/oobabooga/text-generation-webui" target="_blank">
          text-generation-webui
        </Link>{' '}
        instance as a source for local models. Follow{' '}
        <Link href="https://github.com/enricoros/big-agi/blob/main/docs/config-local-oobabooga.md" target="_blank">
          the instructions
        </Link>{' '}
      ...
      {sourceHasLLMs && (
        <Alert variant="soft" color="warning" sx={{ display: 'block' }}>
          ...
          <Link noLinkStyle href="http://127.0.0.1:7860" target="_blank">
            {' '}
            Oobabooga UI
          </Link>
