...
export function OpenAISourceSetup(props: { sourceId: DModelSourceId }) {
  ...
  // external state
  const { source, sourceHasLLMs, access, updateSetup } = useSourceSetup(props.sourceId, ModelVendorOpenAI.getAccess);
    ...
    <>
        ...
        rightLabel={
          <>
            {needsUserKey
              ? !oaiKey && (
                  <>
                    <Link level="body-sm" href="https://platform.openai.com/account/api-keys" target="_blank">
                      create Key
                    </Link>{' '}
                    and{' '}
                    <Link level="body-sm" href="https://openai.com/waitlist/gpt-4-api" target="_blank">
                      apply to GPT-4
                    </Link>
                  </>
                )
      ...
      {advanced.on && (
        <FormTextField
          ...
          description={
            <Link level="body-sm" href="https://platform.openai.com/docs/guides/production-best-practices/setting-up-your-organization" target="_blank">
              What is this
            </Link>
          }
        ...
        <FormTextField
          ...
          description={
            <>
              ...
              ,{' '}
              <Link level="body-sm" href="https://developers.cloudflare.com/ai-gateway/" target="_blank">
                Cloudflare
              </Link>
            </>
          }
      ...
      {advanced.on && (
        <FormTextField
          ...
          description={
            <>
              Generate{' '}
              <Link level="body-sm" href="https://www.helicone.ai/keys" target="_blank">
                here
              </Link>
            </>
          }
      ...
      {!!heliKey && (
        <Alert variant="soft" color={oaiHost?.includes(HELICONE_OPENAI_HOST) ? 'success' : 'warning'}>
          Advanced: You set the Helicone key.{' '}
          {!oaiHost?.includes(HELICONE_OPENAI_HOST)
      ...
      {advanced.on && (
        <FormSwitchControl
          ...
          description={
            <>
              <Link level="body-sm" href="https://platform.openai.com/docs/guides/moderation/moderation" target="_blank">
                Overview
              </Link>
              ,{' '}
              <Link level="body-sm" href="https://openai.com/policies/usage-policies" target="_blank">
                policy
              </Link>
            </>
          }
