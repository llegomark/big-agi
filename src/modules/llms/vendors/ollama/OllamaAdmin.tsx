...
export function OllamaAdmin(props: { access: OllamaAccessSchema; onClose: () => void }) {
  ...
  // external state
  const { data: pullable } = apiQuery.llmOllama.adminListPullable.useQuery(
    { access: props.access },
    {
      staleTime: 1000 * 60,
  ...
  const {
    ...
    mutate: deleteMutate,
    reset: deleteReset,
  } = apiQuery.llmOllama.adminDelete.useMutation();
    ...
    <GoodModal title="Ollama Administration" dividers open onClose={props.onClose}>
      <Stack direction="column" sx={{ gap: settingsGap }}>
        <Typography level="body-sm">
          We assume your Ollama host is running and models are already available. However we provide a way to pull models from the Ollama host, for convenience.
        </Typography>
        ...
        <Box sx={{ display: 'flex', gap: 1, justifyContent: 'space-between' }}>
          <Typography level="body-sm">{pullModelDescription}</Typography>
