...
export function AzureSourceSetup(props: { sourceId: DModelSourceId }) {
  // external state
  const { source, sourceHasLLMs, access, updateSetup } = useSourceSetup(props.sourceId, ModelVendorAzure.getAccess);
  ...
  return (
    <>
      <FormTextField
        ...
        description={
          <Link level="body-sm" href="https://github.com/enricoros/big-agi/blob/main/docs/config-azure-openai.md" target="_blank">
            configuration
          </Link>
        }
      ...
      <FormInputKey
        ...
        rightLabel={
          <>
            {needsUserKey
              ? !azureKey && (
                  <Link level="body-sm" href="https://azure.microsoft.com/en-us/products/ai-services/openai-service" target="_blank">
                    request Key
                  </Link>
                )
