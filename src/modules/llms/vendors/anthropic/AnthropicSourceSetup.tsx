...
export function AnthropicSourceSetup(props: { sourceId: DModelSourceId }) {
  ...
  // external state
  const { source, sourceHasLLMs, access, updateSetup } = useSourceSetup(props.sourceId, ModelVendorAnthropic.getAccess);
  ...
  return (
    <>
      <FormInputKey
        id="anthropic-key"
        label={!!anthropicHost ? 'API Key' : 'Anthropic API Key'}
        rightLabel={
          <>
            {needsUserKey
              ? !anthropicKey && (
                  <Link level="body-sm" href="https://www.anthropic.com/earlyaccess" target="_blank">
                    request Key
                  </Link>
                )
              : '✔️ already set in server'}{' '}
            {anthropicKey && keyValid && (
              <Link level="body-sm" href="https://console.anthropic.com/" target="_blank">
                check usage
              </Link>
            )}
          </>
        ...
        value={anthropicKey}
        onChange={(value) => updateSetup({ anthropicKey: value })}
      ...
      {advanced.on && (
        <FormTextField
          ...
          description={
            <>
              e.g.,{' '}
              <Link level="body-sm" href="https://docs.aws.amazon.com/bedrock/latest/userguide/general-guidelines-for-bedrock-users.html" target="_blank">
                bedrock-claude
              </Link>
            </>
          }
